...
int main(int argc, char *argv[])
{
    ...
    time_signal.set_metadata(
        wss::metadata_builder(time_signal.id(), "Time")
            .data_type(wss::data_types::int64)
            .unit(-1, "seconds", "time", "s")
            .linear_rule(0,
    ...
    {
        wss::local_signal{"/Value1"},
        wss::local_signal{"/Value2"}
    };
    for (unsigned i = 0; i < value_signals.size(); ++i)
        value_signals[i].set_metadata(
            wss::metadata_builder(value_signals[i].id(), "Value" + std::to_string(i + 1))
                .data_type(wss::data_types::real64)
                .unit(-1, "volts", "voltage", "V")
                .range(-10, 10)
